...
public class EnemyMachine : CharacterMachine
{
    ...
    [Header("AI")] [SerializeField] private bool _autoFollow;
    [SerializeField] private float _targetDetectRange;
    [SerializeField] private bool _attackEnable;
    [SerializeField] private float _targeAttackRange;
    ...
    private void UpdateAI()
    {
        switch (_ai)
        {
            ...
            case AI.Think:
            {
                ...
                _target = col ? col.transform : null;

                if (_autoFollow && _target)
                {
                    _ai = AI.Follow;
                }
                else
            ...
            case AI.Follow:
            {
                if (_target == null || Vector2.Distance(transform.position, _target.position) > _targetDetectRange)
                {
                    _ai = AI.Think;
                    return;
                }
                ...
                if (_attackEnable && Vector2.Distance(transform.position, _target.position) <= _targeAttackRange)
                {
    ...
    protected override void OnDrawGizmos()
    {
        ...
        if (_attackEnable)
        {
            ...
            Gizmos.DrawWireSphere(transform.position, _targeAttackRange);    
        }
    }
            ...
            if (collision.TryGetComponent(out CharacterMachine target))
            {
                if (target.isInvincible == false)
                {
                    ...
                    Vector2 knockBackForce =
                        new Vector2((target.transform.position - transform.position).normalized.x, 1.0f);
                    target.Knockback(knockBackForce);
                    DamagePopUp.Create(target.transform.position + Vector3.up * 0.5f, (int)damage, gameObject.layer);
                }
            }
        }
    }
